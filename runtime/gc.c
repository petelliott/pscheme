...
static size_t first_free(const size_t *array, size_t len) {
    for (size_t i = 0; i < len/(sizeof(size_t) * CHAR_BIT); ++i) {
...
static void *allocate_or_null(struct cell_region *region) {
    ...
    size_t i = first_free(region->allocated, CELL_REGION_OBJS);
...
static struct cell_region *make_cell_region(struct cell_region *next) {
    ...
    // we bet that our malloc is fresh and zero-initialized. let's see if this
    // causes problems.
