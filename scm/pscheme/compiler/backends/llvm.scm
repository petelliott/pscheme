(define-library (pscheme compiler backends llvm)
  ...
  (begin
    ...
    (define (builtin-car/cdr off pair)
      (define u (unique))
      (f "    %cadr_~a_int0 = lshr i64 ~a, 4~a\n" u pair (location))
      (f "    %cadr_~a_int = shl i64 %cadr_~a_int0, 4~a\n" u u (location))
      (f "    %cadr_~a_cptr = inttoptr i64 %cadr_~a_int to i64*~a\n" u u (location))
      (f "    %cadr_~a_ptr = getelementptr i64, i64* %cadr_~a_cptr, i64 ~a~a\n" u u off (location))
      (preop)
      (f "load i64, i64* %cadr_~a_ptr~a\n" u (location)))
    ...
    (define (builtin-set-car/cdr! off pair value)
      ...
      (f "    %cadr_~a_cptr = inttoptr i64 %cadr_~a_int to i64*~a\n" u u (location))
      (f "    %cadr_~a_ptr = getelementptr i64, i64* %cadr_~a_cptr, i64 ~a~a\n" u u off (location))
      (f "    store i64 ~a, i64* %cadr_~a_ptr~a\n" value u (location))
      (preop)
      (ret-unspec))
